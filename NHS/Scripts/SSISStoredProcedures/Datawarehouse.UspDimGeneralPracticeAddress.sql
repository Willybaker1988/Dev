...
/****** Object:  StoredProcedure [Datawarehouse].[uspDimGeneralPracticeAddress]    Script Date: 14/01/2018 09:30:30 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [Datawarehouse].[UspDimGeneralPracticeAddress]
...
DECLARE @InsertCount INT = NULL, 
		...
		@vType		  NVARCHAR(100),
		@Comment nvarchar(250) = 'Checking for new [Datawarehouse].[DimGeneralPracticeAddress]  records'
		...
		*/
		...
		(
			SELECT
				...
				[RowBatch]     = ROW_NUMBER() OVER (PARTITION BY t.[GPId], t.[POSTCODE] ORDER BY t.[GPId],t.[FileLogId] desc)
			FROM
